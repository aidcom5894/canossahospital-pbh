<?php
    ...
    if (isset($_POST['update_about'])) { 
        ...
        // पुराना डेटा लाना
        $old_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT avatar FROM ui_obj WHERE id=$id"));
        ...
        $update_query = mysqli_query($conn, "UPDATE ui_obj SET 
    ...
    // 2. डेटा फेच करना
    $result = mysqli_query($conn, "SELECT * FROM ui_obj WHERE id=$id");
    ...
    <div class="container-fluid">
        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
            <div class="card p-3 w-100">
                <h6 class="fw-semibold mb-0"> Edit <span class="text-danger"> / </span> <span class="text-info fw-normal fs-5"> Objectives-Page </span></h6>
            </div>  
        </div>
                            ...
                            <form method="POST" class="form-valide-with-icon needs-validation" enctype="multipart/form-data" autocomplete="off">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <textarea class="form-control" name="about1" placeholder="Enter a objectives heading ..." rows="3" style="resize: vertical;"><?php echo isset($row['about1']) ? htmlspecialchars($row['about1']) : ''; ?></textarea>
                                    </div>
                                </div>

                                <div class="mb-3">
                                    <label class="text-label form-label fw-semibold">About</label>
                                    <div class="input-group">
                                        <span class="input-group-text">
                                            <iconify-icon icon="solar:pen-new-square-outline"></iconify-icon>
                                        </span>
                                        <textarea class="form-control" name="about2" placeholder="Enter Description about objectives..." rows="3" style="resize: vertical;"><?php echo isset($row['about2']) ? htmlspecialchars($row['about2']) : ''; ?></textarea>
                                    </div>
                                </div>
