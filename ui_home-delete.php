...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $imgQuery = "SELECT avatar FROM ui_home WHERE id = '$delete_id' LIMIT 1";
    ...
    $deleteQuery = "DELETE FROM ui_home WHERE id = '$delete_id'";
    $delete = mysqli_query($conn, $deleteQuery);

    if ($delete) {
        // ठीक उसी तरह का बड़ा और परफेक्ट SweetAlert जैसा आपके ui_home.php के सबमिट में था
        echo "
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    ...
                    window.location.href = 'ui_home.php';
                });
            });
        </script>";
        exit();
    } else {
        echo "<script>alert('Error deleting record: " . mysqli_error($conn) . "'); window.location.href='ui_home.php';</script>";
        exit();
    }
} else {
