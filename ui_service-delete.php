...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    if ($delete) {
        // ठीक उसी तरह का बड़ा और परफेक्ट SweetAlert जैसा आपके ui_service.php के सबमिट में था
        echo "
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    ...
                    window.location.href = 'ui_service.php';
                });
            });
        </script>";
        exit();
    } else {
        echo "<script>alert('Error deleting record: " . mysqli_error($conn) . "'); window.location.href='ui_service.php';</script>";
        exit();
    }
} else {
    // अगर बिना ID के कोई आए तो वापस भेजें
    header("Location: ui_service.php");
    exit();
}
?>
